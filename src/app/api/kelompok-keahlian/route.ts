...
export async function GET() {
  ...
  const kelompokKeahlian = await prisma.kelompokKeahlian.findMany({
    orderBy: { nama: "asc" },
  });

  return NextResponse.json({ kelompokKeahlian });
}
...
export async function POST(request: Request) {
  ...
  const parsed = createKelompokKeahlianSchema.safeParse(body);
  if (!parsed.success) {
    return NextResponse.json({ error: parsed.error.flatten() }, { status: 400 });
  }

  const existing = await prisma.kelompokKeahlian.findUnique({
